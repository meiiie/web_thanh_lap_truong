...
const AnimatedButton = ({
  ...
  delay = 0,
  secondary = false,
  transitionType = 'auto', // 'auto', 'slide', 'fade'
}) => {
  const { navigateWithTransition, navigateWithSlide, navigateWithFade } = useViewTransition();
  ...
  useGSAP(
    () => {
      ...
      const initializeAnimation = async () => {
        ...
        // Double-check refs are still available after async operation
        if (!buttonRef.current || !textRef.current || !circleRef.current || !iconRef.current) {
          console.warn('AnimatedButton: Refs not available after font loading');
          return;
        }
  ...
  const handleNavigation = (e) => {
    ...
    // Choose navigation method based on transitionType
  ...
  return (
    <button className={`btn ${secondary ? 'btn-secondary' : ''}`} ref={buttonRef}>
      {buttonContent}
    </button>
  );
};
